// Copy of the verse list in src/utils/dailyScriptureUtils.ts, so the daily
// verse reminder matches the verse of the day on the home screen.
// Keep the two lists the same.

export const dailyScriptures: { verse: string; reference: string }[] = [
    {
    ...
    {
        verse: "There is no fear in love. But perfect love drives out fear, because fear has to do with punishment.",
    ...
    {
        verse: "The Lord has done it this day; let us rejoice today and be glad.",
...
/** Same choice as getTodaysScripture() in the app, for a given local date. */
export function scriptureForDate(year: number, month: number, day: number) {
  const seed = `scripture-${year}-${month}-${day}`;
  let hash = 0;
  for (let i = 0; i < seed.length; i++) {
    hash = (hash << 5) - hash + seed.charCodeAt(i);
    hash |= 0;
  }
  return dailyScriptures[Math.abs(hash) % dailyScriptures.length];
}
