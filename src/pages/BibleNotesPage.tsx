...
const BibleNotesPage = () => {
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] dark:bg-gray-950">
            ...
            <Dialog open={showNoteDialog} onOpenChange={(open) => {
                ...
                <DialogPrimitive.Portal>
                    ...
                    <DialogPrimitive.Content
                        ...
                        className="fixed top-[max(env(safe-area-inset-top),75px)] sm:top-[50px] [@media(display-mode:standalone)]:top-[max(env(safe-area-inset-top),120px)] inset-x-0 bottom-0 w-screen h-[calc(100dvh-max(env(safe-area-inset-top),75px))] sm:h-[calc(100dvh-50px)] [@media(display-mode:standalone)]:h-[calc(100dvh-max(env(safe-area-inset-top),120px))] max-w-none p-0 overflow-hidden bg-white dark:bg-gray-950 z-50 outline-none transform-none sm:rounded-t-[2.5rem] shadow-[0_-8px_30px_rgb(0,0,0,0.12)] border-t border-gray-100 dark:border-gray-800"
                    >
                        {selectedNote && (
            ...
            {showNewNoteDialog && (
                <div className="fixed top-[max(env(safe-area-inset-top),75px)] sm:top-[50px] [@media(display-mode:standalone)]:top-[max(env(safe-area-inset-top),120px)] inset-x-0 bottom-0 w-screen h-[calc(100dvh-max(env(safe-area-inset-top),75px))] sm:h-[calc(100dvh-50px)] [@media(display-mode:standalone)]:h-[calc(100dvh-max(env(safe-area-inset-top),120px))] bg-white dark:bg-black z-[9999] overflow-hidden flex flex-col sm:rounded-t-[2.5rem] shadow-[0_-8px_30px_rgb(0,0,0,0.12)] border-t border-gray-100 dark:border-gray-800">
                    ...
                    <div className="flex-1 flex flex-col w-full relative bg-gray-50 dark:bg-black overflow-hidden">
                        {/* Premium Navbar */}
                        <div className="flex items-center justify-between w-full px-6 h-auto min-h-[4rem] border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm pt-4 pb-4">
