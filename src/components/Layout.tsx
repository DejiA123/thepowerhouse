...
const Layout = ({ children }: LayoutProps) => {
  ...
  return (
    <div className="fixed inset-0 flex flex-col w-full h-[100dvh] min-h-[-webkit-fill-available] text-foreground bg-background overscroll-none overflow-hidden">
      ...
      >
        <div className={cn(
          "min-h-full",
          showChrome && location.pathname !== '/group-chats' ? "pb-16" : (location.pathname === '/group-chats' ? "h-full" : "")
        )}>
          {children}
        </div>
      </main>
