...
const RichTextEditor = forwardRef<RichTextEditorHandle, RichTextEditorProps>(({
  ...
  useEffect(() => {
    ...
    const updateHeight = () => {
      ...
      if (viewport && viewport.height > 100) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: cn(
          'prose max-w-none focus:outline-none text-gray-900 dark:text-gray-100 [&_p]:m-0 [&_p]:min-h-[1.5em] text-[16px]',
          ...
          '[&_.task-item]:flex [&_.task-item]:flex-row [&_.task-item]:items-start [&_.task-item]:gap-3 [&_.task-item]:my-2',
          '[&_.task-item_input]:mt-1 [&_.task-item_input]:w-4 [&_.task-item_input]:h-4 [&_.task-item_input]:shrink-0 [&_.task-item_input]:cursor-pointer',
          '[&_.task-item_p]:m-0 [&_.task-item_div]:m-0',
          '[&_.task-item_label]:mr-0 [&_.task-item_label]:select-none',
  ...
  const renderMenuBar = (position: 'top' | 'bottom' = 'top') => (
    <div className={cn(
      ...
      position === 'bottom'
        ? 'absolute left-0 right-0 bottom-0 flex justify-center pb-[env(safe-area-inset-bottom)] pt-0 z-50'
      ...
      <div className="flex flex-col items-center gap-4 pointer-events-auto">
        ...
        {showInsertMenu && (
          <div className="bg-white/95 dark:bg-gray-950/95 backdrop-blur-2xl border border-gray-100 dark:border-gray-800 p-2 rounded-[2rem] animate-in zoom-in-95 fade-in duration-300 flex items-center gap-2 ring-1 ring-black/5 dark:ring-white/5">
            ...
            <ToolbarButton onClick={() => { setLinkUrl(''); setShowLinkDialog(true); setShowInsertMenu(false); }} title="Add Link">
  ...
  return (
    ...
    >
      ...
      {toolbarPosition === 'bottom' && !readOnly && renderMenuBar('bottom')}
        ...
        <DialogContent>
          ...
          <div className="space-y-4 pt-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={() => { if (linkUrl) { editor.chain().focus().setLink({ href: linkUrl }).run(); setLinkUrl(''); setShowLinkDialog(false); } }}>Add Link</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 pt-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={() => { if (imageUrl) { editor.chain().focus().setImage({ src: imageUrl, alt: imageAlt }).run(); setImageUrl(''); setImageAlt(''); setShowImageDialog(false); } }}>Add Image</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
