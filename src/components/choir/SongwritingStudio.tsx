...
import {
    ...
    Pencil, Save, Check, Share2, Edit3,
} from "lucide-react";
...
const STEPS: { key: string; label: string; icon: any; tip: string }[] = [
    ...
    { key: "compile", label: "Arrange the Song", icon: ListMusic, tip: "Tap ➕ on the ideas you love to lift them into the song above, then drag the wording into shape." },
...
export function SongwritingStudio({ open, onOpenChange, locationId, isAdmin, user }: SongwritingStudioProps) {
    ...
    const [songOpen, setSongOpen] = useState(true);   // the collapsible "song so far" panel
    const [guideOpen, setGuideOpen] = useState(false); // the collapsible step guide
    ...
    const saveName = () => {
        ...
        lsSet("songwriting_name", n); setMyName(n); setEditingName(false);
    ...
    const sendContribution = async () => {
        ...
        } catch (e) { console.error(e); toast.error("Your idea couldn't be sent. Please try again."); }
    ...
    const addToSong = async (c: Contribution) => {
        ...
        try {
            ...
            setSongOpen(true);
            toast.success("Lifted into the song ✨");
    ...
    const focusLabel = STEPS.find((s) => s.key === theme?.focus_step)?.label;
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-3xl gap-0 p-0 h-[100dvh] w-full md:h-[92vh] overflow-hidden border-none bg-slate-50 rounded-none md:rounded-[1.75rem] shadow-2xl z-[201] flex flex-col [&>button]:hidden">
                ...
                <div className="relative shrink-0 overflow-hidden bg-gradient-to-br from-indigo-600 via-violet-600 to-fuchsia-600 px-5 py-4 text-white md:px-6">
                    ...
                    <button onClick={() => onOpenChange(false)} className="absolute right-4 top-4 rounded-full bg-white/15 p-1.5 transition hover:bg-white/25" aria-label="Close"><X className="h-5 w-5" /></button>
                    <div className="relative flex items-center gap-3 pr-10">
                        <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-white/15 backdrop-blur"><Music className="h-6 w-6" /></div>
                        <div className="min-w-0">
                            <h2 className="text-xl font-black tracking-tight md:text-2xl">Songwriting Studio</h2>
                            <p className="truncate text-xs font-medium text-white/80 md:text-sm">Week of {format(new Date(weekDate), "d MMM yyyy")} · write a song together</p>
                        </div>
                    </div>
                    <div className="relative mt-3 flex flex-wrap items-center gap-2">
                        {editingName ? (
                            <div className="flex items-center gap-1.5">
                                <Input autoFocus value={nameInput} onChange={(e) => setNameInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") saveName(); }} placeholder="Your name" maxLength={32}
                                    className="h-8 w-40 border-white/30 bg-white/15 text-sm text-white placeholder:text-white/60" />
                                <Button size="sm" onClick={saveName} className="h-8 bg-white text-indigo-700 hover:bg-white/90"><Check className="h-4 w-4" /></Button>
                            </div>
                        ) : (
                            <button onClick={startEditName} className={cn("flex items-center gap-1.5 rounded-full py-1 pl-1 pr-3 text-xs font-bold transition", myName ? "bg-white/15 hover:bg-white/25" : "animate-pulse bg-white text-indigo-700")}>
                                <span className={cn("grid h-6 w-6 place-items-center rounded-full bg-gradient-to-br text-[10px] font-black text-white", myName ? myColor : "from-indigo-400 to-violet-500")}>{myName ? initialsOf(myName) : "✍️"}</span>
                                {myName || "Tap to add your name"}<Edit3 className="h-3 w-3 opacity-70" />
                            </button>
                        )}
                        {activeWriters.length > 0 && (
                            <div className="ml-auto flex items-center gap-1.5 rounded-full bg-white/15 py-1 pl-2 pr-3 text-xs font-semibold">
                                ...
                                <div className="flex -space-x-1.5">{activeWriters.slice(0, 4).map((w) => (<span key={w.uid} title={w.name} className={cn("grid h-5 w-5 place-items-center rounded-full bg-gradient-to-br text-[8px] font-black text-white ring-1 ring-white/40", w.color || "from-slate-400 to-slate-500")}>{initialsOf(w.name)}</span>))}</div>
                ...
                <div className="shrink-0 border-b border-slate-200 bg-white px-5 py-3 md:px-6">
                    {editingTheme ? (
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-2"><Button size="sm" onClick={saveTheme} className="bg-indigo-600 hover:bg-indigo-700"><Save className="mr-1.5 h-4 w-4" /> Save theme</Button><Button size="sm" variant="ghost" onClick={() => setEditingTheme(false)}>Cancel</Button></div>
                        </div>
                    ) : (
                        <>
                            ...
                            <button onClick={() => setGuideOpen((v) => !v)} className="mt-2 flex items-center gap-1.5 text-xs font-semibold text-indigo-600 hover:text-indigo-700">
                                <Lightbulb className="h-3.5 w-3.5" /> Songwriting guide{focusLabel && <span className="font-normal text-slate-400">· now: {focusLabel}</span>}
                                <ChevronDown className={cn("h-3.5 w-3.5 transition", guideOpen && "rotate-180")} />
                            </button>
                            {guideOpen && (
                                <div className="mt-2">
                                    <div className="flex items-center gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                                        {STEPS.map((step, i) => {
                                            const Icon = step.icon; const isFocus = theme?.focus_step === step.key; const isOpen = openStep === step.key;
                                            return (
                                                <button key={step.key} onClick={() => setOpenStep(isOpen ? null : step.key)}
                                                    className={cn("flex shrink-0 items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-semibold transition",
                                                        isFocus ? "border-indigo-500 bg-indigo-50 text-indigo-700 ring-2 ring-indigo-200" : isOpen ? "border-slate-300 bg-slate-100 text-slate-800" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50")}>
                                                    ...
                                                    <Icon className="h-3.5 w-3.5" />{step.label}{isFocus && <span className="ml-0.5 rounded-full bg-indigo-600 px-1.5 text-[9px] font-bold text-white">NOW</span>}
                                                </button>
                                            );
                                        })}
                                    </div>
                ...
                {/* The Song so far — collapsible panel that grows from the chat */}
                <div className="shrink-0 border-b border-slate-200 bg-white">
                    <div className="flex cursor-pointer select-none items-center gap-2.5 px-4 py-2.5 md:px-5" onClick={() => setSongOpen((v) => !v)}>
                        <span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-600 text-white"><ListMusic className="h-4 w-4" /></span>
                        <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-1.5 text-sm font-bold text-slate-800">The Song so far {orderedLines.length > 0 && <span className="rounded-full bg-violet-100 px-1.5 text-[10px] font-bold text-violet-700">{orderedLines.length}</span>}</div>
                            <p className="truncate text-xs text-slate-400">{lastLine ? `latest: "${lastLine.content}"` : "Empty — write ideas below, then tap ➕ to lift them up here"}</p>
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); copySong(); }} className="rounded-lg p-1.5 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700" title="Copy song"><Copy className="h-4 w-4" /></button>
                        <button onClick={(e) => { e.stopPropagation(); shareSong(); }} className="rounded-lg p-1.5 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700" title="Share song"><Share2 className="h-4 w-4" /></button>
                        <ChevronDown className={cn("h-5 w-5 shrink-0 text-slate-400 transition", songOpen && "rotate-180")} />
                    </div>
                    {songOpen && (
                        <div className="flex max-h-[34vh] flex-col border-t border-slate-100 bg-slate-50/60">
                            <ScrollArea className="min-h-0 flex-1 px-3 md:px-5">
                                <div className="py-3">
                                    {orderedLines.length === 0 ? (
                                        <p className="py-4 text-center text-sm text-slate-400">No lines yet. Promote ideas from the chat with ➕, or add one directly below.</p>
                                    ) : (
                                        orderedLines.map((line, idx) => {
                                            ...
                                            return (
                                                <div key={line.id}>
                                                    {showHeader && (<div className="mb-1 mt-3 flex items-center gap-2 first:mt-0"><span className={cn("h-2.5 w-2.5 rounded-full", meta.dot)} /><span className="text-[11px] font-black uppercase tracking-wider text-slate-500">{meta.emoji} {meta.label}</span></div>)}
                                                    <div className="group flex items-center gap-2 rounded-lg px-1.5 py-1 hover:bg-white">
                                                        ...
                                                        {editing ? (
                                                            <div className="flex flex-1 items-center gap-1.5">
                                                                <Input autoFocus value={lineEditDraft} onChange={(e) => setLineEditDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") saveLineEdit(line); if (e.key === "Escape") setEditingLineId(null); }} className="h-8 flex-1" />
                            ...
                            <div className="shrink-0 border-t border-slate-100 bg-white p-2.5 md:px-5">
                                <div className="mb-1.5"><SectionChips value={lineSection} onChange={setLineSection} /></div>
                                <div className="flex items-center gap-2">
                                    <Input value={lineDraft} onChange={(e) => setLineDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addSongLine(); } }}
                                        placeholder={myName ? `Add a ${SECTION_META[lineSection].label.toLowerCase()} line straight to the song…` : "Add your name above to start writing"} className="h-9 flex-1" />
                                    <Button onClick={addSongLine} disabled={!lineDraft.trim()} className="h-9 shrink-0 bg-gradient-to-br from-violet-500 to-fuchsia-600 hover:opacity-90"><Plus className="h-4 w-4" /></Button>
                                </div>
                            </div>
                        </div>
                    )}
                ...
                {/* Collaboration chat — the main space */}
                <div className="flex min-h-0 flex-1 flex-col">
                    <div className="flex shrink-0 items-center gap-1.5 px-4 pt-2 text-[11px] font-bold uppercase tracking-wide text-slate-400 md:px-5"><MessageCircle className="h-3.5 w-3.5" /> Ideas &amp; collaboration</div>
                    <ScrollArea className="min-h-0 flex-1 px-3 md:px-5">
                        <div className="space-y-3 py-3">
                            {loading ? (
                                <div className="grid place-items-center py-16 text-slate-400"><Loader2 className="h-6 w-6 animate-spin" /></div>
                            ) : contributions.length === 0 ? (
                                <div className="mx-auto mt-8 grid max-w-xs place-items-center gap-3 rounded-2xl border border-dashed border-slate-300 bg-white/60 px-6 py-8 text-center">
                                    ...
                                    <p className="text-sm text-slate-500">Drop a lyric, a hook, or any idea below. Everyone can join in — no login needed. Love an idea? Tap ➕ to lift it into the song above.</p>
                                </div>
                            ) : (
                                contributions.map((c) => {
                                    ...
                                    return (
                                        <div key={c.id} className={cn("flex gap-2", mine ? "flex-row-reverse" : "flex-row")}>
                                            {!mine && <div className={cn("mt-1 grid h-8 w-8 shrink-0 place-items-center self-end rounded-full bg-gradient-to-br text-[11px] font-black text-white", wColor)}>{initialsOf(c.author_name)}</div>}
                                            <div className={cn("group max-w-[80%] rounded-2xl px-3 py-2 shadow-sm", mine ? "rounded-br-sm bg-gradient-to-br from-indigo-500 to-violet-600 text-white" : "rounded-bl-sm border border-slate-100 bg-white text-slate-800")}>
                                                ...
                                                {parent && (
                                                    <div className={cn("mb-1 rounded-lg border-l-2 px-2 py-1 text-[11px]", mine ? "border-white/50 bg-white/10 text-white/80" : "border-indigo-300 bg-indigo-50 text-slate-500")}>
                                                        <span className="font-semibold">{parent.author_name || "Member"}: </span>{parent.content.length > 60 ? parent.content.slice(0, 60) + "…" : parent.content}
                                                    </div>
                                                )}
                                                ...
                                                <div className={cn("mt-1 flex items-center gap-2.5 text-[10px]", mine ? "text-white/70" : "text-slate-400")}>
                                                    ...
                                                    <button onClick={() => toggleLike(c)} className={cn("flex items-center gap-0.5 transition hover:opacity-100", likedByMe ? "opacity-100" : "opacity-70")}><Heart className={cn("h-3 w-3", likedByMe && "fill-rose-400 text-rose-400")} />{likes > 0 && likes}</button>
                                                    ...
                                                    <button onClick={() => addToSong(c)} className="flex items-center gap-0.5 font-semibold opacity-70 transition hover:opacity-100" title="Lift into the song"><Plus className="h-3 w-3" /> Add to song</button>
                    ...
                    <div className="shrink-0 border-t border-slate-200 bg-white p-3 md:px-5">
                        ...
                        <div className="flex items-end gap-2">
                            <Textarea value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendContribution(); } }}
                                placeholder={myName ? `Share a ${SECTION_META[section].label.toLowerCase()} idea…  (Enter to send)` : "Add your name above to start writing"} disabled={sending}
                                rows={1} className="max-h-28 min-h-[44px] flex-1 resize-none rounded-2xl" />
                            <Button onClick={sendContribution} disabled={sending || !draft.trim()} className="h-11 w-11 shrink-0 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 p-0 hover:opacity-90">{sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}</Button>
                        </div>
                    </div>
                </div>
            </DialogContent>
