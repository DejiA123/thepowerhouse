...
// New Bible Brain API service with correct implementation
...
export const bibleBrainServiceNew = {
  async getVersions(): Promise<BibleBrainVersion[]> {
    try {
      ...
      let allVersions: any[] = [];
      let page = 1;
      const maxPages = 20; // Reasonable limit for finding English versions
      ...
      console.log(`✅ Bible Brain: Found ${englishVersions.length} English Bible versions`);
      console.log('📋 Available versions:', englishVersions.map(v => `${v.abbreviation} (${v.name})`));
  ...
  isEnglishVersion(version: any): boolean {
    ...
    return (
      ...
      searchText.includes('new international') ||
      searchText.includes('standard version') ||
      searchText.includes('new living') ||
      searchText.includes('american standard') ||
      searchText.includes('amplified') ||
      languageCode === 'eng' ||
      languageCode === 'en' ||
      // Common English abbreviations
      ['kjv', 'niv', 'esv', 'nlt', 'nasb', 'nkjv', 'amp', 'rsv', 'asv'].some(abbrev => 
  ...
  getFallbackVersions(): BibleBrainVersion[] {
    return [
      { name: 'King James Version', abbreviation: 'KJV', language: 'English', version: 'KJVPCE', source: 'bible-brain' },
      { name: 'English Standard Version', abbreviation: 'ESV', language: 'English', version: 'ESVSRB', source: 'bible-brain' },
      { name: 'New American Standard Bible', abbreviation: 'NASB', language: 'English', version: 'NASB', source: 'bible-brain' },
  ...
  async getChapter(version: string, book: string, chapter: number): Promise<BibleBrainChapter | null> {
    try {
      ...
      if (!bibleInfoResponse.ok) {
        ...
        // Try fallback to KJV
        if (version !== 'KJVPCE') {
          console.log(`🔄 Attempting fallback to KJVPCE for ${book} chapter ${chapter}`);
          return await this.getChapter('KJVPCE', book, chapter);
        }
        return null;
      }
      ...
      const endpoints = [
        // Pattern 1: Direct text endpoint (most common)
      ...
      if (version !== 'KJVPCE') {
        console.log(`🔄 All endpoints failed, attempting fallback to KJVPCE for ${book} chapter ${chapter}`);
        return await this.getChapter('KJVPCE', book, chapter);
      }
